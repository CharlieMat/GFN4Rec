...
# policy arguments
POLICY_CLASS='SlateGFN_TB_Rerank'
INIT_LIST_SIZE=50
INIT_POS_OFFSET=0.1
INIT_NEG_OFFSET=0.1
INIT_LOSS_COEF=0.1
...
do
    for ACTOR_LR in 0.0001 # 0.00003
    do
        for INIT_LOSS_COEF in 0.01 # 0.001 0.1
        do 
            for R_SMOOTH in 0.5 0.3 0.1 0.7
            do
                for SEED in 11 # 13 17 19 23
                do
                    file_key=${AGENT_CLASS}_${POLICY_CLASS}_actor${ACTOR_LR}_R${R_SMOOTH}_F${FWD_OFFSET}_Z${Z}_init${INIT_LOSS_COEF}_c${INIT_LIST_SIZE}_niter${N_ITER}_reg${REG}_ep${INITEP}_seed${SEED}
