...
class SequentialBuffer():
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument('--buffer_random_rate', type=float, default=0,
                            help='probability of random sampling')
        return parser
        
    def __init__(self, args):
        self.buffer_size = args.buffer_size
        self.random_rate = args.buffer_random_rate
        super().__init__()
    ...
    def sample(self, batch_size):
        ...
        '''
        # get indices
        rand_size = int(self.random_rate * batch_size)
        rand_indices = np.random.randint(0, self.current_buffer_size, size = rand_size)
        if self.buffer_head >= batch_size:
            remain_indices = np.arange(self.buffer_head - batch_size + rand_size, self.buffer_head)
        elif self.current_buffer_size < batch_size:
            remain_indices = np.arange(0, self.current_buffer_size)
        else:
            remain_indices = np.arange(0, batch_size - rand_size)
        indices = np.concatenate((rand_indices, remain_indices))
